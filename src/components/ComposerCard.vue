...
<template>
  <div class="group cursor-pointer" @click="emit('click', composer.id)">
    ...
    <div class="space-y-1 px-1">
      <h3 class="font-medium text-sm truncate group-hover:text-foreground transition-colors">{{ composer.name || 'Unknown Composer' }}</h3>
      <p class="text-xs text-foreground opacity-60">Composer</p>
    </div>
  </div>
</template>
