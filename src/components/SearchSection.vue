...
const rowsCount = computed(() => props.rows)
...
function updateColumns() {
  if (sectionRef.value) {
    ...
    const itemFullWidth = props.rows > 1 ? 240 : 200
    const gap = 24
    columnsPerPage.value = Math.max(1, Math.floor((width + gap) / itemFullWidth))
  }
}
  ...
  <section ref="sectionRef" v-if="items.length > 0" class="animate-in fade-in slide-in-from-bottom-4 duration-700">
    ...
    <div class="relative overflow-hidden min-h-[120px]">
      <Transition :name="transitionName">
        <div :key="currentPage" class="grid gap-x-6 gap-y-4 w-full"
          :style="{
            gridTemplateColumns: `repeat(${columnsPerPage}, minmax(0, 1fr))`,
            gridTemplateRows: `repeat(${rowsCount}, minmax(0, auto))`,
            gridAutoFlow: 'column'
          }">
          <div v-for="(item, index) in paginatedItems" :key="(item as any).id || index">
