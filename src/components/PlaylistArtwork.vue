...
const props = defineProps<{
  playlist: { id: string; artwork_key?: string }
  tracks?: { artwork_key?: string; album?: { artwork_key?: string } }[]
}>()
...
<template>
  <div class="w-full h-full relative overflow-hidden flex-shrink-0 flex items-center justify-center bg-foreground/[0.03]">
    ...
    <template v-if="playlistArtworks.length === 1 || (playlistArtworks.length > 1 && playlistArtworks.length < 4)">
      <LazyImg :src="playlistArtworks[0]" class="w-full h-full object-cover" />
    </template>
    
    <!-- 4-Grid Fallback -->
    <template v-else-if="playlistArtworks.length >= 4">
      <div class="grid grid-cols-2 grid-rows-2 w-full h-full">
        <LazyImg v-for="key in playlistArtworks.slice(0, 4)" :key="key" :src="key" class="w-full h-full object-cover" />
      </div>
    </template>
