    ...
    <template v-if="visible">
      ...
      <div ref="menuEl"
        ...
        <template v-for="(item, index) in items" :key="index">
          <div v-if="item.separator" class="my-1 mx-2 border-t border-border-glass" />
          <div v-else
            class="relative flex items-center gap-3 px-3 py-1.5 text-sm cursor-default transition-all rounded-lg mx-0.5"
            :class="[
              item.disabled ? 'text-foreground opacity-40 cursor-not-allowed'
                : item.danger ? 'text-error hover:text-error hover:bg-error/20'
                  : 'text-foreground opacity-80 hover:text-foreground hover:bg-foreground/15',
            ]"
            @click="handleItemClick(item)" @mouseenter="handleMouseEnter(item, index, $event)">
      ...
      <div v-if="activeSubmenuIndex !== null && items[activeSubmenuIndex]?.children?.length" ref="submenuEl"
        ...
        <div v-for="(child, ci) in items[activeSubmenuIndex]!.children" :key="ci"
